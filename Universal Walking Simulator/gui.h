...
DWORD WINAPI GuiThread(LPVOID)
{
	...
	while (!done)
	{
		...
		{
			ImGui::Begin(("ballsfn"), nullptr, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoTitleBar);
			...
			{
				if (ImGui::BeginTabItem(ICON_FA_GAMEPAD " Game Config"))
				{
					Tab = GAME_TAB;
					PlayerTab = -1;
				...
				{
					if (ImGui::BeginTabItem(ICON_FA_PEOPLE_CARRY " Edit Players"))
					{
						Tab = PLAYERS_TAB;
						ImGui::EndTabItem();
					}
				...
				if (ImGui::BeginTabItem(("Set Gamemode")))
				{
					Tab = GAMEMODE_TAB;
					PlayerTab = -1;
					bInformationTab = false;
				...
				if (bIsLateGame && ImGui::BeginTabItem(("Enable Lategame")))
				{
					Tab = LATEGAME_TAB;
					PlayerTab = -1;
					bInformationTab = false;
				...
				if (ImGui::BeginTabItem("Dumping Tab"))
				{
					Tab = DUMP_TAB;
					PlayerTab = -1;
					bInformationTab = false;
				...
				if (ImGui::BeginTabItem(("UWS Credits")))
				{
					Tab = CREDITS_TAB;
					PlayerTab = -1;
					bInformationTab = false;
			...
			if (PlayerTab == -1)
			{
				...
				case GAME_TAB:
				{
					...
					if (FnVerDouble == 12.41 && ImGui::Button("Open All Vaults (c2)"))
					{
						FortAI::OpenVaults();
					}
					
					// https://media.discordapp.net/attachments/998297579857137734/1006634482884939807/unknown.png
					if (bAISpawningEnabled && ImGui::Button("Spawn AIs"))
					{
					...
					if (/* Engine_Version == 420 && */ ImGui::Button("Summon Items"))
					{
						LootingV2::SummonFloorLoot(nullptr);
					}

					if (false && ImGui::Button("Summon Vehicles"))
					{
					...
					if (serverStatus == EServerStatus::Up)
					{
						if (ImGui::Button("Ingame Debug Camera"))
						{
						...
						if (ImGui::Button("Vending Machine Loot"))
						{
					...
					*/
					...
					if (/* bStarted && */ ImGui::Button("Restart Server"))
					{
						Restart();
					}

					if (bAISpawningEnabled && ImGui::Button(("Spawn Henchmans"))) {
						FortAI::SpawnHenchmans();
					}

					if (false && Engine_Version >= 422 && Engine_Version < 424) 
					{
						if (ImGui::Button("Spawn Barrier (stay in 1 place to get creative inventory)"))
						{
							auto World = Helper::GetWorld();
							auto NetDriver = *World->Member<UObject*>(("NetDriver"));
				...
				case GAMEMODE_TAB:
				{
					...
					if (ImGui::Checkbox(("Enable Playground (inf mats, ammo, and respawning)"), &bIsPlayground))
					{
					...
					if (!bIsPlayground && Engine_Version < 424)
						ImGui::Checkbox(("Enable Lategame (ending storm circles, can be enabled with playground)"), &bIsLateGame);
				...
				case EVENT_TAB:
					...
					if (FnVerDouble == 8.51) {
						...
						if (ImGui::Button("Unvault Item (Unvaulting Event)")) {
							FString InStr;
					...
					if (FnVerDouble == 12.41 && ImGui::Button("Fling Up (Travis)"))
					...
					if (FnVerDouble == 6.21)
					{
						...
						if (ImGui::Button("Teleport Players to Butterfly (Butterfly Event)"))
				...
				case LATEGAME_TAB:
					...
					{
						ImGui::Text("Storm has not started yet!");

						if (serverStatus == EServerStatus::Up && 
							ImGui::Button("Start Storm"))
						{
							FString StartAircraftCmd;
				...
				case SETTINGS_TAB:
					...
					{
						ImGui::InputText("CID (SkinID)", &CIDToUse);
						ImGui::NewLine();
					}
					...
					if (std::floor(FnVerDouble) > 4)
						ImGui::Checkbox("Random Skins", &bRandomCosmetics);
				...
				case CREDITS_TAB:
					...
					TextCentered(("Practic: Simplified GUI."));
				...
				if (PlayerTab < Players.size())
				{
					...
					if (CurrentPlayer.first && CurrentPlayer.second)
					{
						...
						if (!bInformationTab)
						{
							...
							static std::string KickReason = "Operator kicked you from the game.";
							...
							ImGui::InputText("OP Kick", &KickReason);
							if (ImGui::Button("Kick Players"))
							{
							...
							if (ImGui::Button("Summon Wea[pms"))
							{
								auto wID = FindObject(WID);
							...
							if (FnVerDouble == 6.21)
							{
								if (ImGui::Button("Spawn Zombie (has no AI)"))
								{
